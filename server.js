...
const SECRET = 'agsus-secret-2025'; // Em produção use env var
...
app.post('/api/login', (req, res) => {
  ...
  const token = jwt.sign({ id: user.id, email: user.email, nome: user.nome }, SECRET, { expiresIn: '24h' });
...
app.post('/api/esqueci-senha', (req, res) => {
  res.json({ mensagem: 'Funcionalidade futura (SMTP).' });
});
...
app.get('/api/usuarios', verificarToken, (req, res) => {
  try {
    const lista = db.prepare('SELECT id, nome, email, criadoem FROM usuarios ORDER BY nome').all();
    res.json(lista);
  } catch (e) {
...
app.post('/api/usuarios', verificarToken, (req, res) => {
  ...
  try {
    ...
    registrarLog(req.usuario.id, 'CRIAR_USUARIO', `Criou usuário: ${email}`);
...
app.delete('/api/usuarios/:id', verificarToken, (req, res) => {
  ...
  if (id === req.usuario.id) return res.status(400).json({ erro: 'Você não pode se excluir.' });

  try {
    const userAlvo = db.prepare('SELECT email FROM usuarios WHERE id = ?').get(id);
    if (!userAlvo) return res.status(404).json({ erro: 'Usuário não encontrado' });

    db.prepare('DELETE FROM usuarios WHERE id = ?').run(id);
    registrarLog(req.usuario.id, 'DELETAR_USUARIO', `Removeu usuário: ${userAlvo.email}`);
  ...
  try {
    const resultado = db.transaction(() => {
      ...
      if (seq) {
        ...
        db.prepare('INSERT INTO sequencias (tipo, ano, ultimonumero) VALUES (?, ?, ?)').run(dados.tipo, anoAtual, 1);
      }
